...
class PlayerPerGameDataHelper: DataHelperProtocol {
    static let TABLE_NAME = "PlayerPerGame"
    static let table = Table(TABLE_NAME)
    static let playerSeasonPerGameID = Expression<Int64>("playerSeasonPerGameID")
    ...
    typealias T = PlayerPerGame
    
    static func find(id: Int64) throws -> T? {
        guard let DB = SQLiteDataStore.sharedInstance.db else {
            throw DataAccessError.Datastore_Connection_Error
        }
        let query = table.filter(playerSeasonPerGameID == id)
        let items = try DB.prepare(query)
        for item in  items {
            return PlayerPerGame(playerSeasonPerGameID: item[playerSeasonPerGameID],
    ...
    static func findAll() throws -> [T]? {
        ...
        for item in items {
            print("\(item)")
            retArray.append(PlayerPerGame(playerSeasonPerGameID: item[playerSeasonPerGameID],
                playerName: item[playerName],
                Season: item[Season],
                Age: item[Age],
                Tm: item[Tm],
